...
def split_patterns(restriction_text):
    ...
    p2 = re.compile(
        r"(II|III|IV|V|VI)(?:\/)(\d+[a-z]?)\s*(?:,|and)\s*(\d+[a-z]?)", re.IGNORECASE
    )
    p3 = re.compile(
        r"entry\s+(\d+[a-z]?)(?:\s+and\s+(\d+[a-z]?))?\s+of\s+Annex\s+II", re.IGNORECASE
    )
...
def process_data(scraper, input_name):
    ...
    for item in matched_items:
        ...
        if isinstance(restriction_raw, list):
        ...
        if (
            annex_nos
            and annex_refs
            and len(annex_nos) == len(annex_refs)
            and annex_nos[0] != "None"
        ):
            for idx, annex_roman in enumerate(annex_nos):
                ...
                if match_meta:
                    ...
                    prod_types.append(
                        header + (", ".join(pt) if isinstance(pt, list) else str(pt))
                    )
                    max_concs.append(
                        header + (", ".join(mc) if isinstance(mc, list) else str(mc))
                    )
                    opinions.append(
                        header + (", ".join(sc) if isinstance(sc, list) else str(sc))
                    )
